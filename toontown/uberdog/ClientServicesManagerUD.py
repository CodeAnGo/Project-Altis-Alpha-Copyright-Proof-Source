...
class LocalAccountDB:
    ...
    def lookup(self, cookie, callback):
        ...
        # Edit: This is clearly needed because of cunts faking play tokens, so here we go
        httpReq = httplib.HTTPConnection('www.projectaltis.com')
        httpReq.request('GET', '/api/validatetoken?t=%s' % (cookie))
        ...
        if cookie.__len__() != 64: # Cookies should be exactly 64 Characters long!
            callback({'success': False,
